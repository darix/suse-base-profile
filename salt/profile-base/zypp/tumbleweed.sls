...
{{ repo_id }}:
  ...
  {%- if repo in ["oss", "non-oss"] %}
    ...
    {%- if repo == "non-oss" %}
      ...
      {%- set update_repo_id = "repo-update-" ~ repo       %}
      {%- set debug_repo_id  = "repo-debug-" ~ repo        %}
      {%- set source_repo_id = "repo-source-" ~ repo       %}
    {%- else %}
      {%- set update_dir     = update_basedir              %}
      {%- set update_repo_id = "repo-update"               %}
      {%- set debug_repo_id  = "repo-debug"                %}
      {%- set source_repo_id = "repo-source"               %}
    {%- endif %}
